  ...
  client.on('message', msg => {
    ...
    if (regex.test(msg.content)==true) {
      if (!msg.member.hasPermission("ADMINISTRATOR")) {
        ...
        return msg.reply('Bu sunucuda linkler **Better Bot** tarafından engellenmektedir.Link atmana izin vermeyeceğim!').then(msg => msg.delete(3000));
      } else {
        return;
      };
    } else {
...
client.on('ready', () => {
  ...
  client.setInterval(() => {
      client.user.setActivity("şakir-help | Yardım Menüsüne Göz Atabilirsiniz", { type: "WATCHING" });
      client.user.setActivity("şakir-tavsiye | Tavsiyenizi Bize Bildirebilirsiniz", { type: "WATCHING" });
    client.user.setActivity("şakir-oynat | Eklendi Muzik Dinleyebilirsiniz", { type: "WATCHING" });
      client.user.setActivity("Yeni Altyapı Efsane Komutlar", { type: "WATCHING" });  
          client.user.setActivity("şakir-canlı-destek | Bizimle Canlı Canlı İletişime Geçin", { type: "WATCHING" });  
      client.user.setActivity(`${client.guilds.size} Sunucu | ${client.users.size} Kullanıcı Ya Hizmet Veriyoruz`, { type: "WATCHING" });
  }, 1000);
});
